...
def ask_personal_info():
    ...
    exercise = input("How will you exercise? ").strip().lower()
...
def compute_tdee(bmr):
    ...
    return tuple(math.ceil(bmr * f) for f in factors)
...
def acquire_meal_summary(meal_num, entree, entree_cal, dessert, dessert_cal,
    ...
    """
    meal_cal = entree_cal + dessert_cal + drink_cal
    return ( 
        f"\nMeal #{meal_num} Summary:\n"
        f"Entree: {entree}, {entree_cal} kcal.\n"
        f"Dessert: {dessert}, {dessert_cal} kcal.\n"
        f"Drink: {drink}, {drink_cal} kcal.\n"
        f"Meal calories: {meal_cal} kcal.\n"
        f"Cumulative calories: {total_cal_sum} kcal.")
...
def compute_excess_calories(consumed_calories, needed_calories):
    """ Compute and return the difference between consumed and needed calories.
        ...
        419
    """
    dif = consumed_calories - needed_calories
...
def compute_exercise_duration(_exercise, _excess_cal):
    ...
    if _excess_cal <= 0:
        return 0
    burned_per_hour = BURNED_CALORIES_PER_HOUR[_exercise]
    hours_needed = _excess_cal / burned_per_hour
...
def acquire_exercise_summary(_exercise, _excess_cal):
    ...
    """
    if _excess_cal <= 0:
        return 'You do not need to remove excess calories.'
    minutes = compute_exercise_duration(_exercise, _excess_cal)
    return (f'You consumed {_excess_cal} kcal in excess and need to do {_exercise} for {minutes} minutes.')
...
def display_summary(_exercise, total_cal_sum, activity_level, tdee):
    ...
    header = (f'\nIf you are {level_text}, your suggested daily calories are {tdee} kcal.')
    
    excess = compute_excess_calories(total_cal_sum, tdee)
    tail = acquire_exercise_summary(_exercise, excess)
    return header + '\n' + tail
...
if __name__ == "__main__":
    ...
    t1, t2, t3, t4, t5 = compute_tdee(bmr)
    ...
    print(display_summary(exercise, day_calories, 1, t1))
    print(display_summary(exercise, day_calories, 2, t2))
    print(display_summary(exercise, day_calories, 3, t3))
    print(display_summary(exercise, day_calories, 4, t4))
    print(display_summary(exercise, day_calories, 5, t5))